...
def get_url_info(config, log_path, tmp_dir_name):
    ...
    #                     errors += 1
    # logging.info('\tFinished. lines: %d; erros: %d', lines, errors)
    # if lines == 0:
    #     logging.warning('No lines in the log file {!s}'.format(log_path))
    # if errors / lines > config['PARSE_ERROR_RATIO']:
...
def get_report_info(config, collect_file_name, tmp_dir_name):
    ...
    logging.info('Extract stat data from %s to %s', collect_file_name, stat_file_name)
    ...
    with open(str(collect_file_name), 'rt') as collect_file:
        with open(str(stat_file_name), 'w') as stat_file:
            ...
            for line in collect_file:
                ...
                if match is None:
                    msg = 'Invalid line "{:s}" in {!s}'.format(line, collect_file_name)
                    logging.critical(msg)
                    raise LAE(msg)
                if url is None or url != match.group(1):
                    if url is not None:
                        ...
                        print('{:s}\t{:d}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}'.format(url, stat['count'], stat['time_sum'], stat['time_avg'], stat['time_max'], stat['time_med']), file=stat_file)
                    url = match.group(1)
                else:
